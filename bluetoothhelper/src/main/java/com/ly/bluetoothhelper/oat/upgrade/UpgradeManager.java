...
import com.ly.bluetoothhelper.oat.upgrade.packet.VMUPacket;
...
public class UpgradeManager {
     ...
     * {@link com.ly.bluetoothhelper.oat.upgrade.codes.OpCodes.Enum#UPGRADE_DATA_BYTES_REQ UPGRADE_DATA_BYTES_REQ} request.
     */
    ...
    private void sendVMUPacket(VMUPacket packet, boolean isTransferringData) {
        ...
        if (isUpgrading) {
            ...
            Log.w(TAG, "Sending failed as application is not anymore upgrading for opcode: " + OpCodes.getString
                    (packet.getOpCode()));
        }
    }
    ...
    private void onFileUploadProgress () {
        ...
        if (mStartOffset > 0) {
            ...
            long elapsedTime = System.currentTimeMillis() - mTimeStartTransfer;

            UploadProgress progress = new UploadProgress(percentage, elapsedTime);
     ...
     * {@link com.ly.bluetoothhelper.oat.upgrade.codes.OpCodes.Enum#UPGRADE_DATA_BYTES_REQ} request.
     */
