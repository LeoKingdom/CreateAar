...
public class BleConnectHelper1 {
     ...
     * @return
     */
    public BleConnectHelper1 init(Application application, int reconnectCount, double maxDistance, int operatime) {
        bluetoothHelper = new BluetoothHelper(application, reconnectCount, operatime);
        bleManager = bluetoothHelper.getBleManager();
        this.maxDistance = maxDistance;
        return this;
    }
                ...
                for (String mac : needConnectMap) {
                    if (!bleManager.isConnected(mac)) {
                        if (macaddrList.contains(mac)) {
                            ...
                            bluetoothHelper.connect(bleDevice.getMac(), connectListener);
...
//                            }
                        ...
                        connDeviceMap.put(mac,bluetoothHelper.getBleDeviceFromMac(mac));
                    }
